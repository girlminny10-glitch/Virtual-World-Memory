...
ALTER TABLE npc_pair_conversations DISABLE ROW LEVEL SECURITY;
ALTER TABLE world_objects DISABLE ROW LEVEL SECURITY;
-- Desabilitar RLS para as tabelas do jogo (necessário para o servidor gravar)
ALTER TABLE npc_memories DISABLE ROW LEVEL SECURITY;
ALTER TABLE npc_creations DISABLE ROW LEVEL SECURITY;
ALTER TABLE npc_learnings DISABLE ROW LEVEL SECURITY;
ALTER TABLE npc_relationships DISABLE ROW LEVEL SECURITY;
...
CREATE TABLE IF NOT EXISTS npc_creations (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  npc_id TEXT NOT NULL, description TEXT NOT NULL, type TEXT NOT NULL,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
CREATE TABLE IF NOT EXISTS npc_learnings (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  npc_id TEXT NOT NULL, learning TEXT NOT NULL,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE(npc_id, learning)
);
CREATE TABLE IF NOT EXISTS npc_relationships (
  npc_id TEXT PRIMARY KEY, data JSONB NOT NULL DEFAULT '{}',
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
CREATE TABLE IF NOT EXISTS npc_pair_conversations (
  pair_key TEXT PRIMARY KEY, history JSONB NOT NULL DEFAULT '[]',
  topic TEXT NOT NULL DEFAULT '', updated_at TIMESTAMPTZ DEFAULT NOW()
);
CREATE TABLE IF NOT EXISTS world_objects (
  id TEXT PRIMARY KEY, creator TEXT NOT NULL, creator_id TEXT NOT NULL,
  creator_color TEXT NOT NULL, type TEXT NOT NULL, description TEXT NOT NULL,
  position_x FLOAT NOT NULL, position_z FLOAT NOT NULL,
  color TEXT NOT NULL DEFAULT '#aaaaaa', scale FLOAT NOT NULL DEFAULT 1.0,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
ALTER TABLE npc_memories DISABLE ROW LEVEL SECURITY;
ALTER TABLE npc_creations DISABLE ROW LEVEL SECURITY;
