...
const OBJ_TYPES = [
  "house","tower","fountain","garden","monument","chair","table","lamp_post","arch","pyramid",
  "totem","well","bench","crystal","portal","statue","painting","car","boat","tree",
  "rock","fence","gate","swing","mushroom","star_monument","flower_bed","bridge","dome","spiral",
...
function spawnObj(obj: WorldObject, scene: THREE.Scene, ref: Record<string, THREE.Group>) {
  ...
  if (t==="house"||t==="cabin") {
    ...
    add(new THREE.Mesh(new THREE.ConeGeometry(3.2*sc,2*sc,4), m(0x884422)), (3+1)*sc); grp.children[1].rotation.y=Math.PI/4;
...
export default function Game() {
  ...
  const [chatTarget, setChatTarget] = useState<string|null>(null); // npc id or "all"
  ...
  const npcsR      = useRef<Record<string,{group:THREE.Group;state:NpcState&{targetPosition?:Pos};bubble?:THREE.Sprite;bTimer?:number}>>({});
  ...
  const toast = useCallback((text:string, color:string) => {
    ...
    setToasts(p=>[...p,{id,text,color}].slice(-3));
    setTimeout(()=>setToasts(p=>p.filter(t=>t.id!==id)), 4500);
  },[]);

  const addMsg = useCallback((msg: Msg) => {
    setMsgs(p=>[...p,msg].slice(-40));
  ...
  const updateMinimap = useCallback(() => {
    ...
    Object.values(objsR.current).forEach(o=>{ ctx.fillStyle="#334"; ctx.fillRect(off+o.position.x*sc-1,off+o.position.z*sc-1,2,2); });
  ...
  useEffect(()=>{
    ...
    scene.background = new THREE.Color(0x080818); scene.fog = new THREE.Fog(0x080818,100,320);
    ...
    const sun = new THREE.DirectionalLight(0xfff0dd,1.2); sun.position.set(80,150,60); sun.castShadow=true;
    sun.shadow.mapSize.set(2048,2048); sun.shadow.camera.left=-200; sun.shadow.camera.right=200; sun.shadow.camera.top=200; sun.shadow.camera.bottom=-200; sun.shadow.camera.far=600;
    scene.add(sun); scene.add(new THREE.HemisphereLight(0x223366,0x112244,.4));
    ...
    scene.add(new THREE.GridHelper(700,90,0x1a251a,0x111a11)); // subtle grid

    // Stars
    const sb=new Float32Array(2400*3); for(let i=0;i<7200;i+=3){sb[i]=(Math.random()-.5)*700;sb[i+1]=50+Math.random()*300;sb[i+2]=(Math.random()-.5)*700;}
    const stG=new THREE.BufferGeometry(); stG.setAttribute("position",new THREE.BufferAttribute(sb,3));
    scene.add(new THREE.Points(stG,new THREE.PointsMaterial({size:.45,color:0xffffff,transparent:true,opacity:.6})));
    ...
    const showBubble = (npcId:string, text:string, color:string, emote:string) => {
      const n=npcsR.current[npcId]; if(!n) return;
      if(n.bubble) scene.remove(n.bubble);
      const sp=makeBubble(text,color,emote);
    ...
    wsRef.current = ws;
    ws.onopen = () => ws.send(JSON.stringify({type:"player-name",name:pName,gender:pGender}));

    ws.onmessage = (e) => {
      try { const d=JSON.parse(e.data); handleWS(d); } catch{}
    };

    const handleWS = (d: Record<string,unknown>) => {
      switch(d.type as string) {
        ...
        case "npc-move": {
          ...
          if(n&&d.targetPosition) { n.state.targetPosition=d.targetPosition as Pos; n.state.emotion=d.emotion as string; }
          break;
        }
        case "npc-arrived": {
          const n=npcsR.current[d.npcId as string];
          if(n&&d.position){n.group.position.set((d.position as Pos).x,0,(d.position as Pos).z);n.state.targetPosition=undefined;}
          break;
        }
        case "npc-conversation": {
          const emF=(d.fromEmotion as string)?.match(/\p{Emoji}/u)?.[0]??"💬";
          const emT=(d.toEmotion as string)?.match(/\p{Emoji}/u)?.[0]??"💬";
          showBubble(d.fromId as string,d.message as string,d.fromColor as string,emF);
          setTimeout(()=>showBubble(d.toId as string,d.response as string,d.toColor as string,emT),1200);
          setFeed(p=>[{fromName:d.from as string,fromColor:d.fromColor as string,toName:d.to as string,toColor:d.toColor as string,message:d.message as string,response:d.response as string,ts:Date.now()},...p].slice(0,20));
          setNpcList(p=>p.map(n=>n.id===d.fromId?{...n,emotion:d.fromEmotion as string}:n.id===d.toId?{...n,emotion:d.toEmotion as string}:n));
          break;
        }
        case "npc-thought": {
          const em=(d.emotion as string)?.match(/\p{Emoji}/u)?.[0]??"💭";
          showBubble(d.npcId as string,d.thought as string,d.npcColor as string,em);
          break;
        }
        case "npc-created-object": {
          spawnObj(d.object as WorldObject,scene,objsR.current);
          const em=(d.emotion as string)?.match(/\p{Emoji}/u)?.[0]??"✨";
          showBubble(d.npcId as string,d.description as string,d.npcColor as string,em);
          toast(`${d.npcName} criou: ${d.description}`, d.npcColor as string);
          break;
        }
        ...
        case "npc-greet-player": {
          const em=(d.emotion as string)?.match(/\p{Emoji}/u)?.[0]??"👋";
          showBubble(d.npcId as string,d.message as string,d.npcColor as string,em);
          if(d.targetPlayerId===pidRef.current){
            setMention({name:d.npcName as string,text:d.message as string});
            setTimeout(()=>setMention(null),6000);
          }
          break;
        }
        case "npc-response": {
          const npc=npcList.find?.(n=>n.id===d.npcId)||Object.values(npcsR.current).find(n=>n.state.id===d.npcId)?.state;
          const em=(d.emotion as string)?.match(/\p{Emoji}/u)?.[0]??"💬";
          showBubble(d.npcId as string,d.response as string,d.npcColor as string,em);
          ...
          setNpcList(p=>p.map(n=>n.id===d.npcId?{...n,emotion:d.emotion as string}:n));
          setTimeout(()=>chatMsgsRef.current?.scrollTo(0,99999),50);
          if(d.response && (d.response as string).toLowerCase().includes("@"+pName.toLowerCase())){
            setMention({name:d.npcName as string,text:d.response as string});
    ...
    const onKey = (e:KeyboardEvent,down:boolean) => {
      ...
      if(down&&k==="escape"){ setDrawOn(false); setChatTarget(null); }
    };
    ...
    // Animation
    let animId:number, lastT=performance.now(), mmT=0;
    const animate=(t:number)=>{
      ...
      if(dir.lengthSq()>0){
        ...
        playerR.current.pos.x+=dir.x*js; playerR.current.pos.z+=dir.z*js;
        playerR.current.group.position.set(playerR.current.pos.x,0,playerR.current.pos.z);
      ...
      camera.position.set(pg.position.x+cs.dist*Math.sin(cs.h)*Math.cos(cs.v),pg.position.y+cs.dist*Math.sin(cs.v),pg.position.z+cs.dist*Math.cos(cs.h)*Math.cos(cs.v));
      ...
      Object.values(npcsR.current).forEach(n=>{
        if(n.state.targetPosition){
          const tx=n.state.targetPosition.x-n.group.position.x, tz=n.state.targetPosition.z-n.group.position.z;
          const dd=Math.sqrt(tx*tx+tz*tz);
          if(dd>.4){ n.group.position.x+=tx/dd*5*dt; n.group.position.z+=tz/dd*5*dt; n.group.rotation.y=Math.atan2(tx,tz); }
          else { if(n.state.targetPosition){ n.group.position.set(n.state.targetPosition.x,0,n.state.targetPosition.z); n.state.targetPosition=undefined; } }
        }
        if(n.bubble){
      ...
      // Objects scale-in
      Object.values(objsR.current).forEach(o=>{ if(o.scale.x<1){const s=Math.min(1,o.scale.x+dt*1.6);o.scale.set(s,s,s);} });
  ...
  const sendChat = () => {
    ...
    if(chatTarget==="all"){
      ...
      const npc=npcList.find(n=>n.id===chatTarget);
  ...
  const chatNpc = npcList.find(n=>n.id===chatTarget);
  ...
  return (
    <div style={{width:"100dvw",height:"100dvh",overflow:"hidden",background:"#080818",position:"relative",fontFamily:"system-ui,sans-serif",userSelect:"none"}}>
      ...
      {uiOn && (<>
        ...
        <div className="hud" style={{position:"absolute",top:10,left:"50%",transform:"translateX(-50%)",zIndex:100,background:"rgba(4,4,16,0.82)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:20,padding:"5px 16px",fontSize:12,color:"#88aaff",fontWeight:700,whiteSpace:"nowrap",boxShadow:"0 2px 12px rgba(0,0,0,.4)"}}>
          {pGender==="female"?"👩":"👨"} {pName}
          <span style={{color:"#334",fontSize:10,fontWeight:400,marginLeft:10}}>WASD · drag · U=UI · C=desenho</span>
        </div>
              ...
              {npcList.map(n=>(
                <div key={n.id} onClick={()=>{setChatTarget(n.id);setTab("chat");}}
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{color:"#445",fontSize:10,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{n.emotion}</div>
                  </div>
        ...
        {/* ── Draw toolbar (only when drawing) ── */}
        {drawOn && (
        ...
        {/* ── Toasts ── */}
        <div style={{position:"absolute",top:10,left:"50%",transform:"translateX(-50%)",zIndex:250,display:"flex",flexDirection:"column",gap:5,alignItems:"center",pointerEvents:"none",marginTop:40}}>
          {toasts.map(t=>(
            <div key={t.id} style={{background:"rgba(4,4,16,0.93)",border:`1px solid ${t.color}33`,borderLeft:`3px solid ${t.color}`,borderRadius:10,padding:"6px 14px",color:"#bbc",fontSize:11,maxWidth:260,textAlign:"center",animation:"fdi .3s",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
              {t.text}
            </div>
          ))}
        </div>
      ...
      {/* ── UI hidden mode: minimal toggle button ── */}
      {!uiOn&&(
      ...
      {/* ── Joystick (always visible on mobile) ── */}
