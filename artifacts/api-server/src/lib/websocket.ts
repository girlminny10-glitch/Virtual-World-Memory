...
import {
  npcs, players, worldObjects, worldObjectIdCounter, respondToPlayer,
  setBroadcast, npcMove,
} from "./world";
...
export function createWebSocketServer(server: import("http").Server): WebSocketServer {
  ...
  _wss.on("connection", (ws: WebSocket, req: IncomingMessage) => {
    ...
    players[playerId] = { id: playerId, name: "Viajante", position: { x: 0, z: 0 } };
    ...
    ws.on("message", async (raw) => {
      ...
      if (data.type === "player-name") {
        if (players[playerId]) {
          players[playerId].name = String(data.name ?? "Viajante");
        }
      }
      ...
      if (data.type === "player-chat") {
        ...
        const reply = await respondToPlayer(npc, String(data.message));
        if (reply) {
          broadcast({
            type: "npc-response",
            npcId: npc.id,
      ...
      if (data.type === "player-create") {
        ...
        const obj = {
          id: `obj-${Date.now()}`,
          creator: players[playerId]?.name ?? "Viajante",
          creatorId: playerId,
          creatorColor: "#4ECDC4",
          type: String(data.objType),
          description: `${players[playerId]?.name ?? "Viajante"} construiu um(a) ${data.objType}`,
          position: pos,
          createdAt: Date.now(),
        };
        worldObjects.push(obj);
        if (worldObjects.length > 200) worldObjects.shift();
        broadcast({ type: "npc-created-object", object: obj, npcName: obj.creator, npcId: playerId, npcColor: "#4ECDC4", emotion: "feliz" });
      }
    });
